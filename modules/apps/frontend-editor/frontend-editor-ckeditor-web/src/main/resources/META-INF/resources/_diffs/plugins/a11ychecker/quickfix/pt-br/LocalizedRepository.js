 ...
 */
define(['quickfix/Repository'], (Repository) => {
	'use strict';
	 ...
	 */
	LocalizedRepository.prototype.deferGetCall = function (
		langCode,
		getArguments
	) {
		...
		if (indexOf(languagesRequested, langCode) === -1) {
			...
			CKEDITOR.scriptLoader.load(
				this.basePath + 'lang/' + langCode + '.js'
			);
		}
	...
	LocalizedRepository.prototype._addDeferredGet = function (
		langCode,
		getArguments
	) {
	...
	LocalizedRepository.prototype._getDeferredGetCount = function (langCode) {
		return deferredGetCalls[langCode]
			? deferredGetCalls[langCode].length
			: 0;
	};
