...
(function () {
	...
	CKEDITOR.plugins.a11ychecker.quickFixes.get({
		...
		callback(ElementReplace) {
			...
			ParagraphToHeader.prototype.fix = function (
				formAttributes,
				callback
			) {
				var that = this;
				ElementReplace.prototype.fix.call(this, formAttributes, () => {
					that._removeBoldTag();
					callback(that);
				});
			};
			...
			ParagraphToHeader.prototype._getPreferredLevel = function (editor) {
				...
				while ((prevElement = walker.previous())) {
					if (
						prevElement.getName &&
						prevElement.getName().match(headerTagRegExp)
					) {
			...
			ParagraphToHeader.prototype._removeBoldTag = function () {
				var isElementEvaluator = function (el) {
					...
					hasSingleElement =
						innerElement &&
						innerElement.equals(elem.getLast(isElementEvaluator)),
			...
			ParagraphToHeader.prototype._getPossibleLevels = function (editor) {
				...
				for (i = tags.length - 1; i >= 0; i--) {
					...
					if (
						!tags[i].match(/^h[1-6]$/i) ||
						!editor.filter.check(tags[i])
					) {
						tags.splice(i, 1);
					}
					else {
			 ...
			 * @param {CKEDITOR.editor} editor
			 */
			ParagraphToHeader.prototype._getFormHeaderLeves = function (
				editor
			) {
				var dict = {},
