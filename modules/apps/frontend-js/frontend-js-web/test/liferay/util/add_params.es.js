...
describe('Liferay.Util.addParams', () => {
	...
	it('Adds the parameters to the portlet URL', () => {
		expect(addParams({bar: 'foo', foo: 'bar'}, sampleUrl)).toEqual(
			'http://sampleurl.com?bar=foo&foo=bar'
		);
	});
	...
	it('Sets anchor hash after base url and parameters', () => {
		expect(
			addParams({bar: 'foo', foo: 'bar'}, anchorHashSampleUrl)
		).toEqual('http://sampleurl.com?bar=foo&foo=bar#foo');
	});

	it('Sets portlet url when base url has parameters', () => {
		expect(
			addParams({bar: 'foo', foo: 'bar'}, sampleUrlWithParams)
		).toEqual('http://sampleurl.com?param&bar=foo&foo=bar');
	});
});
